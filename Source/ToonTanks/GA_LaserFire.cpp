    ...
    const FGameplayEventData* TriggerEventData)
{
    ...
    if (!CommitAbility(Handle, ActorInfo, ActivationInfo)) // 檢查冷卻/消耗條件
    {
        UE_LOG(LogTemp, Warning, TEXT("Can't spell"));
    ...
    if (!ASC)
    {
        UE_LOG(LogTemp, Warning, TEXT("Can't Find ASC"));
        return;
    }

    // It's done at BP
    //if (ConsumeAmmoEffect)
    //{
    //    ASC->ApplyGameplayEffectToSelf(
    //        ConsumeAmmoEffect->GetDefaultObject<UGameplayEffect>(),
    //        1.0f,
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("Can't Find AmmoEffect"))
    }
